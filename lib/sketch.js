...
let cameraCanvas = function (sketch) {
  ...
  sketch.draw = function () {
    ...
    if (pose) {
      ...
      rightAngle = getAngle([pose.leftShoulder.x, pose.leftShoulder.y], [pose.rightShoulder.x, pose.rightShoulder.y], [pose.rightWrist.x, pose.rightWrist.y]);
      leftAngle = getAngle([pose.rightShoulder.x, pose.rightShoulder.y], [pose.leftShoulder.x, pose.leftShoulder.y], [pose.leftWrist.x, pose.leftWrist.y]); // sketch.textSize(30);
...
var moleCanvas = function (sketch) {
  ...
  let spritedata;
  let iconPlay;
  const animation = {}; // const canvasWidth = 500;

  const canvasWidth = sketch.displayWidth < 700 ? sketch.displayWidth : 500; // const canvasHeight = 700;

  const canvasHeight = sketch.displayWidth < 700 ? sketch.displayHeight - 80 : 700; // Timer variables

  let timer = 30; // Game state
  ...
  sketch.setup = function () {
    ...
    let frames = spritedata.frames; // load sprite by animation
  ...
  sketch.draw = function () {
    ...
    if (isPlaying) {
      moles.map(mole => mole.draw()); // check left pose click
      ...
      if (sketch.dist(sketch.mouseX, sketch.mouseY, canvasWidth / 2, canvasHeight / 2 + 20) < 30) {
    ...
    sketch.text("Developed by Steve Chan | Image assets found in lepunk's repo", canvasWidth / 2, canvasHeight - 5);
    sketch.frameRate(8);
  };
  ...
  sketch.mouseClicked = function () {
    ...
    if (isPlaying) {
      ...
      const d = sketch.dist(sketch.mouseX, sketch.mouseY, canvasWidth / 2, canvasHeight / 2 + 20);
